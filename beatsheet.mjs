...
function renderEl(b) {
  switch (b.kind) {
    ...
    case "floatcard":
      return (
        `<FloatingCard durationInFrames={d} image=${j(b.image)} title=${j(b.title || "")}` +
    ...
    case "impact2":
      return (
        `<BigImpact durationInFrames={d} image=${j(b.image)} lines={${j(b.lines || [])}}` +
        (b.align ? ` align=${j(b.align)}` : ``) +
        (b.hue ? ` hue=${j(b.hue)}` : ``) +
        ` />`
      );
    case "bubbles":
      return (
        `<FloatingBubbles durationInFrames={d} image=${j(b.image)} bubbles={${j(b.bubbles || [])}}` +
        (b.side ? ` side=${j(b.side)}` : ``) +
        (b.hue ? ` hue=${j(b.hue)}` : ``) +
        ` />`
      );
    case "infocard":
      return (
        `<InfoCard durationInFrames={d} image=${j(b.image)} title=${j(b.title || "")} items={${j(b.items || [])}}` +
        (b.side ? ` side=${j(b.side)}` : ``) +
        (b.hue ? ` hue=${j(b.hue)}` : ``) +
        ` />`
      );
...
if (kinds.has("floatcard")) imports.push(`import { FloatingCard } from "./scenes/FloatingCard";`);
{
  const _bold = [];
  if (kinds.has("impact2")) _bold.push("BigImpact");
  if (kinds.has("bubbles")) _bold.push("FloatingBubbles");
  if (kinds.has("infocard")) _bold.push("InfoCard");
  if (_bold.length) imports.push(`import { ${_bold.join(", ")} } from "./scenes/Bold";`);
}
